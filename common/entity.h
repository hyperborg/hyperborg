...
public:
	...
	_disable_copy = false;
	qDebug() << "ENT::entityChangeRequested";
        emit entityChangeRequested(reqvalues);
    }
...
signals:
    void entityChanged();
    void entityChangeRequested(QHash<QString, QVariant> list);
    void entityChanged(QHash<QString, QVariant> list);
...
protected slots:
	...
	values = lst;
	emit entityChanged();
        emit entityChanged(values);
    }

private:
    QHash<QString, QVariant> values;		// current values for this entity
