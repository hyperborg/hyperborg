...
constexpr int NODE_RESTART_CODE = 2222;
...
constexpr auto PATH_GLOBAL = "GLOBAL";
...
using ParameterList = QHash<QString, QVariant>;
...
public:
    HVariant() = default;
    HVariant(QVariant v, Unit u) : value(std::move(v)), unit(u) {}
...
{
    static const QStringList chk = { "YES", "1", "TRUE", "ENABLED", "OK", "JA" };
    return chk.contains(str.toUpper());
}

static double convert(int from, int to, double src_val)
{
    if (from == to) return src_val;
...
public:
    ...
    SensorInfo(QString _key, DataType _dt, Unit _unit = NotDefined, int _precision = 5, int _scale = 2)
        : key(std::move(_key)), datatype(_dt), unit(_unit), precision(_precision), scale(_scale) {
    }
...
{
    static constexpr int pow10[5] = { 1, 10, 100, 1000, 10000 };   // Faster to execute than pow()
...
static bool splitKeyAndVal(const QString& src, QString& key, QString& val)
{
    const QStringList wsl = src.split("=");
    if (wsl.count() == 2)
    {
        key = wsl.at(0);
        val = wsl.at(1);
